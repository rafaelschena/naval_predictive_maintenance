...
df_comp <- read.csv('compressor_cenarios.csv')
...
(p3 <- p2 + geom_text_repel(data = df_comp[df_comp$Cenário==1,], aes(label = Algoritmo, 
                                                               hjust = 1, vjust = 1))
          + ggtitle("Evolução de Acurácia x Recall_Emerg Compressor")
...
df_turb <- read.csv('turbina_cenarios.csv')

q1 <- ggplot(df_comp, aes(Cenário, Acuracia.Recall, group = Algoritmo, colour = Algoritmo))
